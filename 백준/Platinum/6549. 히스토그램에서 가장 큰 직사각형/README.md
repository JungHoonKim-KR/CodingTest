...
### 풀이
제일 큰 막대를 기준으로 분할하려고 했는데, 그냥 중간 부터 하면 됐었다. 생각해보니 분할 문제에서 굳이 제일 큰 값을 고려해서 분할하지 않고 항상 중간값을 기준으로 나눴는데 왜 그런 생각을 했는지 모르겠다.<br>
여기서 중요한 건 왼쪽, 오른쪽만 비교하지 말고 전 범위를 고려한 넓이까지 비교해야 한다는 점.
