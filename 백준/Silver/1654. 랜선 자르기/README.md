...
### 풀이
문제에 접근 조차 하지 못했다. 길이를 이분탐색해야겠다는 생각을 아예 못했다.<br>
만약 길이를 생각했다면 한 가지 함정이 더 있다. 최대값을 구하기 때문에 uppderBound를 썼을 텐데, 배열과 달리 길이로 비교하기 때문에 만약 모든 랜선의 길이가 같다면 최대값 +1에서 탐색이 종료되어야 하지만 그렇지 못하기 때문에<br>
직접 max++을 해줘야 한다
