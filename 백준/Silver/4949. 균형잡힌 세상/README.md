...
### 풀이
코테라도 로직에 맞게 메서드화 하는 것이 좋다는 것을 깨닫게 해준 문제.<br>
코테는 성능이 우선이니 더티코딩을 해도 상관 없다고 생각했는데, 로직이 길어질 수록 메서드화를 하지 않으면<br>
if else 및 지역변수를 남발하게 되는 경우가 많아졌다. 그래서 메서드화로 깔끔하게 로직을 분리하는 것이 더 좋은 것 같다.
