...
### 풀이
연산자를 순회해야 한다고는 생각했는데、 dfs로 완전탐색을 한다는 생각을 못했다。 처음에는 브루스포스로 완전탐색 하려고 했는데、 이러면 연산자 순서를 바꾸는게 쉽지 않을 것 같았다。
앞으로 완전탐색은 DFS를 우선으로 생각하는게 편할 것 같다。
