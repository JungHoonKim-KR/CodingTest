...
### 풀이
아이디어 자체는 맞았다. 최소값을 가지는 도시를 먼저 찾아가려고 Map에 <price,index> 를 저장한 후 price로 정렬했다. 그리고 최소값 도시를 먼저 찾아가 index ~ n까지 모두 계산하고 반복했다. <br>
이렇게 하면 정답은 맞으나, 시간 초과가 발생한다. 왜 굳이 복잡하게 했을까....<br>
어차피 0번 인덱스 부터 해당 도시 가격에 맞게 계산을 해야하기 때문에 처음부터 진행하면서 최소값을 갱신했으면 금방 푸는 문제다.
