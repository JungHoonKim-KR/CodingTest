...
주의할 점
1. 각 물건 당 개수는 1개임
2. 칸 채울 때 현재 무게에 대해서만 채우지 말고 이전 데이터를 비교하며 가져와야됨.
   즉 만약 현재 물건의 무게가 아직 배낭 용량보다 크다면
   ```java
   else if(i!=0)knapback[i][weight] = knapback[i-1][weight];
   ```
   이전 데이터를 그대로 반영해야한다는 것. 그렇지 않으면 최대 가치 상태를 보전할 수 없음.
