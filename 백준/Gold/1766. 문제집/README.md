...
### 회고
3665번 문제는 문제의 순서가 우선시 된 후 일부 바뀌는 것을 고려하는 것.
반대로 이 문제는 조건 순서를 보면 2번 조건인 선행 문제를 푸는 것이 우선, 그 다음 3번 조건이 가능한 쉬운 문제 즉 오름차순 정렬이다.
그렇기 때문에 이 문제는 3665처럼 모든 간선을 연결하는 것이 아니라 선행 조건 간의 그래프 및 차수는 고려하고, 우선순위 큐로 오름차순으로 결과를 출력하는 것.
