...
### 풀이
전깃줄을 만든 후 겹치는 가장 많이 겹치는 전깃줄들을 뺀 후 또 가장 많이 겹치는 전깃줄을 체크해서 빼는 것을 반복하려 했으나, 너무 복잡했다.<br>
그러나 이것 외에는 도저히 생각이 안나서 블로그를 참조했는데, 참조한지 10초 만에 답이 나왔다.<br>
애초에 전깃줄이 없는 상태에서 한 개씩 설치하면 이전에 푼 LIS와 같은 문제가 된다.<br>
앞으로 문제를 풀 때 다 만들어진 상태에서 제거하는 건 배제하고 생각해야겠다.(너무 복잡함)
